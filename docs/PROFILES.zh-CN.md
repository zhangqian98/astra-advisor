...
每个任务只使用一个基础 Profile，最多加一个修饰器。实际读写边界决定
`EXPLORE` 或 `WORK`，任务类型再决定是否增加修饰器；最终 review 固定为 `REVIEW`：

| 边界或任务类型 | Profile | 行为重点 |
| --- | --- | --- |
| 任意只读、非 review 任务 | `EXPLORE` | 追踪真实路径，给出文件、符号或工件证据。 |
| 分配了精确可写文件的任务 | `WORK` | 最小范围修改，保持合同，跑针对性检查。 |
| debug | 加 `+DEBUG` | 先复现，再验证竞争假设，根因有证据前不做大改。 |
| test | 加 `+TEST` | 验证可观察行为，报告准确命令和结果，只改已分配文件。 |
| docs | 加 `+DOCS` | 核对权威版本文档；只有任务明确分配文档文件时才写入。 |
